...
for k = numCol : -1 : 1
  ...
  if(~isempty(ind) & ~isempty(ind2))
    ...
    % Outer product 
    % Ainv(ind,ind2) = Ainv(ind,ind2) - ...
      % Ainv(ind,k)*Afactor(k,ind2);
    % % Inner product
    % Ainv(k,ind2) = Ainv(k,ind2) - Ainv(k,ind) * Afactor(ind,ind2);
