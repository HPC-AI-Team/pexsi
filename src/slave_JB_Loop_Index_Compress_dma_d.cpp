...
extern "C" void  JB_Loop_index_compress_dma_d(JB_Loop_param_d_t* param){
    ...
    for( int jb = local_j_start; jb < local_j_end; jb++ ){
        for( int ib = local_i_start; ib < local_i_end; ib++ ){
            ...
            for( int ibSinv = 0; ibSinv < LcolSinv.len; ibSinv++ ){
                ...
                double buffer[MAX_BLOCK_SIZE];

                // Transfer the values from Sinv to AinvBlock
                double* nzvalSinv = SinvB->nzval;
                int ldSinv    = SinvB->numRow;
                for( int j = 0; j < UB->numCol; j++ ){
                  double* nzvalAinv_j = &nzvalAinv[j*ldAinv];
                  double* nzvalSinv_j = &nzvalSinv[relCols[j] * ldSinv];
                  // for( int i = 0; i < LB.numRow; i++ ){
                  ...
                  for(int ptr = 0; ptr < segment_compress.segment_count; ++ptr){
                      ...
                      double* nzvalSinv_j_offset = nzvalSinv_j + offset;
                      // #pragma omp simd
                      // for(int i = i_start; i < i_end; i++){
                      //     nzvalAinv_j[i] = nzvalSinv_j_offset[i];
                      // } 
                      athread_dma_get(buffer, nzvalSinv_j_offset + i_start, (i_end - i_start) * sizeof(double));
                      athread_dma_put(nzvalAinv_j + i_start, buffer, (i_end - i_start) * sizeof(double));
                    //   memcpy(nzvalAinv_j + i_start,nzvalSinv_j_offset + i_start,(i_end - i_start) * sizeof(double));
                  }
                }               
