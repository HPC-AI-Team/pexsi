    ...
    public:
      ...
      void ForwardMessage( char * data, size_t size, int tag, MPI_Request * requests ){
        for( Int idxRecv = 0; idxRecv < myDests_.size(); ++idxRecv ){
          ...
          int error_code = MPI_Isend( data, size, MPI_BYTE, 
    ...
    class TreeReduce: public TreeBcast{
      ...
      public:
        ...
        inline bool IsDone(){
          ...
          if (retVal && myRoot_ != myRank_ && fwded_){
            ...
            MPI_Test(&sendRequest_,&flag,&stat);
            int error_code = stat.MPI_ERROR;
            if (error_code != MPI_SUCCESS) {
        ...
        virtual bool Progress(){
          ...
          if(isReady_ && !retVal){
            ...
            int all_error_code = MPI_Testsome(reqCnt,&myRequests_[0],&recvCount,&recvIdx_[0],&myStatuses_[0]);


            if (all_error_code != MPI_SUCCESS) {
              if(recvCount>0){
                for(Int i = 0;i<recvCount;++i ){
                  Int idx = this->recvIdx_[i];
                  int error_code = this->myStatuses_[i].MPI_ERROR;
                  if (error_code != MPI_SUCCESS) {
                    char error_string[BUFSIZ];
                    int length_of_error_string, error_class;
                    MPI_Error_class(error_code, &error_class);
        ...
        virtual void PostFirstRecv()
        {
          if(this->GetDestCount()>this->numRecvPosted_){
            for( Int idxRecv = 0; idxRecv < myDests_.size(); ++idxRecv ){
              Int iProc = myDests_[idxRecv];
              //assert(msgSize_>=0);
              int error_code = MPI_Irecv( (char*)remoteData_[idxRecv], msgSize_, MPI_BYTE, 
      ...
      protected:
        ...
        void Forward(){ 
          ...
          // Use Isend to send to multiple targets
          if(myData_==NULL){
            error_code = MPI_Isend( NULL, 0, MPI_BYTE, 
            ...
            error_code = MPI_Isend( (char*)myData_, msgSize_, MPI_BYTE, 
                iProc, tag_,comm_, &sendRequest_ );
#ifdef COMM_PROFILE
      ...
      public:
        ...
        virtual void PostFirstRecv()
        {
          ...
          if(this->isAllocated_ && this->GetDestCount()>this->numRecvPosted_){
            int error_code = MPI_Irecv( (char*)this->remoteData_[0], this->msgSize_, MPI_BYTE, 
                this->myDests_[0], this->tag_,this->comm_, &this->myRequests_[0] );
            this->numRecvPosted_++;
            if (error_code != MPI_SUCCESS) {
              char error_string[BUFSIZ];
              int length_of_error_string, error_class;
        ...
        virtual bool Progress(){
          ...
          if(this->isReady_ && !retVal){
            ...
            int all_error_code = MPI_Testsome(reqCnt,&this->myRequests_[0],&recvCount,&this->recvIdx_[0],&this->myStatuses_[0]);
