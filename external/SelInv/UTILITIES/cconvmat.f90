...
!       ****************************************************************
!
!       CCONVMAT.F converts the Matrix Market (MM) format to the 
!       ccf format that cselinv.x uses. This subroutine utilizes mmio.f 
!       provided by Matrix Market.
!
!       usage: cconvmat.x -infile=<input file name> -outfile=<output file name>
!
...
!       to .ccf format, run
!
!       convertmat.x -infile=Hc.mtx -outfile=Hc.ccf
...
if( nargc == 0 ) then
  ...
  write(0, *) 'usage cconvmat.x -infile=<input file name> -outfile=<output file name>'
  write(0, *) 
  ...
  write(0, *)
  write(0, *) 'cconvmat.x -infile=Hc.mtx -outfile=Hc.ccf'
  stop
end if
do i = 1, nargc
  call getarg(i, str)
...
nnzmax = entries
allocate(indx(nnzmax), jndx(nnzmax), cval(nnzmax))
...
! Convert the single complex format to double complex format
! allocate(dcval(entries))
! do k=1, entries
  ! dcval(k) = cval(k)
! enddo
...
write(ounit) (cval(k), k = 1, entries)
close(ounit)

deallocate(indx,jndx,cval, colptr)
! deallocate(dcval)
end program
