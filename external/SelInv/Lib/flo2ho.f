...
c
c     NZVALS   COMPLEX*16 array of size COLPTR(N+1)-1. (INPUT)
...
c
c     NEWVALS  COMPLEX*16 array of size H_NNZ (OUTPUT)
...
cdebug
c      funit = 20
c      open(unit=funit,file='lap2d.ccf',form='unformatted')
c      write(funit) n, colptr(n+1)-1
c      write(funit) (colptr(i), i = 1, n+1)
c      write(funit) (rowind(i), i = 1, colptr(n+1)-1)
c      write(funit) (nzvals(i), i = 1, colptr(n+1)-1)
c      close(unit=funit)
c
