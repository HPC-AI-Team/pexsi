...
cd $1
ls comm_stat* | grep -v tgz | ../parse_avg 0 $nproc 1  > $outdir/bcastU_total.dat 2>/dev/null
#ls comm_stat* | grep -v tgz | ../parse_avg 0 $nproc 3  > bcastL_total.dat 2>/dev/null
ls comm_stat* | grep -v tgz | ../parse_avg 0 $nproc 4  > $outdir/reduceL_total.dat 2>/dev/null
ls comm_stat* | grep -v tgz | ../parse_avg 0 $nproc  > $outdir/total.dat 2>/dev/null
ls comm_stat* | grep -v tgz | ../parse_avg 1 $nproc  > $outdir/avg.dat 2>/dev/null
